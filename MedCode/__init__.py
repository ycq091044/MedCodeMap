...
class CodeMapping:
    ...
    def load_drugbank(self):
        if not os.path.exists(f'{self.path}/name2SMILES.csv'):
            name2SMILES = pd.read_csv(
                request.urlopen('https://drive.google.com/uc?id=199i8mP2gMQNhwUe-2ZNmIr5fhiBbzVlK'))
    ...
    def load_RxNorm2Name(self):
        if not os.path.exists(f'{self.path}/RxNorm2Name.csv'):
            RxNorm2Name = pd.read_csv(
                request.urlopen('https://drive.google.com/uc?id=1Z11J4st1sI44jPborls9jIxzcpF-GpGt'))
            RxNorm2Name.to_csv(f'{self.path}/RxNorm2Name.csv', index=False)
            print('source loaded from https://drive.google.com/uc?id=1Z11J4st1sI44jPborls9jIxzcpF-GpGt')
        else:
            RxNorm2Name = pd.read_csv(f'{self.path}/RxNorm2Name.csv')
            print(f'source loaded from {self.path} cache')
        self.RxNorm_to_Name = self.mapping_from_pd(RxNorm2Name, 'RxNorm', 'Name')
        self.G.add_edge('RxNorm', 'Name')

    def load_RXCUI2atc4(self):
        if not os.path.exists(f'{self.path}/RXCUI2atc4.csv'):
            RXCUI2atc4 = pd.read_csv(
                request.urlopen('https://drive.google.com/uc?id=1I2G6fsBDXDiAK95qFWwtnl3Ib2MaLeCx'))
            RXCUI2atc4.to_csv(f'{self.path}/RXCUI2atc4.csv', index=False)
            print('source loaded from https://drive.google.com/uc?id=1I2G6fsBDXDiAK95qFWwtnl3Ib2MaLeCx')
        else:
            RXCUI2atc4 = pd.read_csv(f'{self.path}/RXCUI2atc4.csv')
            print(f'source loaded from {self.path} cache')
    ...
    def load_rxnorm2RXCUI(self):
        if not os.path.exists(f'{self.path}/rxnorm2RXCUI.txt'):
            ...
            with open(f'{self.path}/rxnorm2RXCUI.txt', 'w') as outfile:
            ...
            infile = open(f'{self.path}/rxnorm2RXCUI.txt', 'r')
